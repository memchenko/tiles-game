...
import {
  MatrixCalculator,
  IRendereableMatrix,
} from "../../lib/grid/matrix-calculator";
import { Renderer, drawSolidRect } from "../../lib/grid/renderer";
import { DPI, isLandscape } from "../../constants/device";

export const TilesGrid = React.memo(function TilesGrid({
  matrix,
}: ITilesGridProps) {
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer<string>(drawSolidRect);
  ...
  return (
    <div className="tiles-grid" ref={container}>
      <canvas ref={canvas} className="tiles-grid__canvas"></canvas>
    </div>
  );
});
