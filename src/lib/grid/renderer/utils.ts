...
export const get2DContext = (canvasElement: HTMLCanvasElement) =>
  new IO(() => {
    return canvasElement.getContext("2d");
  });

export const getCanvasIO = (selector: string) =>
  getElement(selector)[chain](get2DContext);

export const getCanvasGeometry = (ctx: CanvasRenderingContext2D) =>
  new IO(() => {
...
export const displayImage = curry(
  ({ ctx, colWidth, rowHeight }: IGridContext, cell: ICell<string>) =>
    new IO(() => {
      ...
      ctx.drawImage(
        cell.payload as unknown as CanvasImageSource,
        cell.x,
        cell.y,
        colWidth,
        rowHeight
...
export const setColor = ({ ctx }: IGridContext, cell: ICell<string>) =>
  new IO(() => {
    if (!cell.payload) return cell;
  ...
  ({ ctx, colWidth, rowHeight }: IGridContext, cell: ICell<string>) =>
    new IO(() => {
      ...
      ctx.quadraticCurveTo(
        ...
        displayY + displayHeight,
        displayX + displayWidth,
        displayY + displayHeight - radius
      );
      ctx.lineTo(displayX + displayWidth, displayY + radius);
      ctx.quadraticCurveTo(
        displayX + displayWidth,
        displayY,
        displayX + displayWidth - radius,
        displayY
      );
      ctx.closePath();
      ctx.fill();
...
export const drawSolidRect = curry(
  (context: IGridContext, cell: ICell<string>) =>
    setColor(context, cell)[chain](displayRect(context)).unsafePerformIO()
);

export const drawGrid = <T>(
  drawRect: (rect: ICell<T>) => void,
  mtx: ICell<T>[][]
) => mtx.forEach((row) => row.forEach(drawRect));

export const redrawColumn = <T>(
  drawRect: (rect: ICell<T>) => void,
  ...
  const drawableArr = [prependant]
    .concat(arr.map((el) => ({ ...el })))
    .concat([appendant]);
